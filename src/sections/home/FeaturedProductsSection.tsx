      ...
      <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 xl:px-12">
        ...
        >
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white">
              ...
              <span className="text-[#DC2626]">{t('featuredProductsHighlight')}</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {featuredProducts.map((product, index) => {
            ...
            const offersForProduct = productOffers.filter(
              (offer) => offer.productId === product.id
            );

            const minPrice =
              offersForProduct.length > 0
                ? Math.min(...offersForProduct.map((offer) => offer.price))
                : product.price;

            const shopsCount = offersForProduct.length;
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-[#DC2626]">
                        {t('startsFrom')} {minPrice} {t('currencyQAR')}
                      </span>
                    </div>
